...
# --- Directores: listar alumnos de su carrera ---
...
# --- Docentes: listar alumnos de sus grupos ---
...
# --- Alumnos: listar sus calificaciones ---
...
def crear_aspirante(db: Session, aspirante: AspiranteCreate):
    nuevo = Aspirante(
        ...
        def listar_materias_por_docente(db: Session, idUsuario: int):
            # obtener grupos del docente y buscar materias asignadas a esos grupos
            grupos = db.query(Grupo).filter(Grupo.idDocente == idUsuario).all()
            ids_grupos = [g.idGrupo for g in grupos]
            if not ids_grupos:
                return []
